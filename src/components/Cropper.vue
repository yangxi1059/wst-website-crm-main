<template>
  <div class="custom-upload" >
    ...
    <el-upload
      ...
      <!-- <div class="cropper__tip" slot="tip">只能上传jpg/png/jpeg文件</div> -->
    <!-- <input
      :id="id"
      type="file"
      name="single"
...
export default {
  ...
  methods: {
    ...
    async onChange(file,fileList) {
      ...
      try {
        // 读取文件
        const src = await files.readFile(file)
    ...
    uploadCover() {
      this.$refs.cropper.getCropBlob(async data  => {
        try {
          // 文件大小限制
          if (!files.isMaxFileSize(data , this.maxFileSize)) {
            return
          }
          this.$loading()
          console.log(data )
          ...
          let file = new window.File([data],fileName,{type: data.type})
           files.uploadImgBackUrlCropper(fileName,file, url => {
            console.log(url);
            setTimeout(() => {
