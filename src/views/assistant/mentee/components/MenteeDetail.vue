...
<template>
  ...
  >
    ...
    >
      ...
      <div v-if="menteeEditType == 2">
        <div class="mentee_detail_item" >
          <el-tag size="medium"  class="mentee_detail_item_tag" >激活</el-tag>
          ...
          <el-form-item label="分配新顾问" prop="counselorWxNew">
            <el-select
              :style="{width:widths}"
              v-model="menteeDetail.counselorWxNew"
              clearable
              filterable
              placeholder="请选择"
              @change = "counselorChange(1)"
            >
            <!-- 不可激活时不可编辑 -->
              <el-option
          ...
          <el-form-item
            label="分配新顾问日期"
            prop="counselorDateNew"
          >
            <el-date-picker
              :style="{width:widths}"
              v-model="menteeDetail.counselorDateNew"
              :picker-options="pickerOptions"
              :clearable="false"
              :editable="false"
              type="date"
          ...
          <el-form-item label="激活截图上传" class=""  >
          <div class="activate_pic" v-if="activateUrl!==''">
            <el-image style="width: 120px; height: 120px" :src="activateUrl" fit="contain" :preview-src-list="[activateUrl]" ></el-image>
            <div class="shadow" @click="removeUrl()">
              <i class="el-icon-delete"></i>
            </div>
          </div>
          <div class="activate_pic" v-else>
            <App-cropper
              :width="400"
              :height="301"
              :img-tab = "'detailArr'"
...
export default {
    ...
    menteeDetailVisible: function (val) {
      if (val) {
        ...
        if (this.menteeEditType == 1) {
          ...
          this.remote(this.menteeDetail.sourceName)
          ...
          if (this.menteeDetail.channelId == 'channel-kol') {
            this.setList(this.menteeDetail.sourceName)
          }
          this.$nextTick(() => {
        ...
        if (this.menteeEditType == 2) { this.title = '学员顾问分配激活' }
      }
    },
  ...
  methods: {
    ...
    changeSourceFrom () {
      this.$set(this.menteeDetail, 'sourceFrom', this.menteeDetail.sourceFrom)
    ...
    submit () {
      ...
      if (this.menteeEditType == '2') {
        ...
        this.menteeDetail.counselorStatus = 1
      }
      if (this.menteeDetail.counselorStatus == 1) {
      ...
      const data2 = JSON.parse(JSON.stringify(this.menteeDetail))
      data2.major = data2.major.join(',')
      console.log(data2, 1111111111111111111)
      if (data2.graduateMajor && Array.isArray(data2.graduateMajor)) {
        data2.graduateMajor = data2.graduateMajor.join(',')
      } else {
        data2.graduateMajor = ''
      }
