<template>
  <div class="hr-user-edit">
    <el-dialog :close-on-click-modal="false"
      title="活动经费重申"
      :visible.sync="reapplyCooperatorActivityFundVisible"
      width="900px"
      :before-close="close"
    >
...
export default {
  ...
  props: {
    reapplyCooperatorActivityFundVisible: {
      type: Boolean,
      default: false
    },
    contractData: {
  ...
  watch: {
    reapplyCooperatorActivityFundVisible: function (val) {
      if (val) {
  ...
  methods: {
    ...
    initPage () {
      ...
      this.activityId = this.contractData.info.activityId
      this.activityName = this.contractData.info.activityName
    ...
    submit () {
      ...
      const data = {
        ...
        applyTitle: `（重申）BD[${this.cooperatorName}]活动(${this.activityName})的经费申请`,
        copyTo: copyTo,
        approval: approval,
        content: {
          file: this.file,
      ...
      if (this.fileList.length) {
        ...
        for (let i = this.fileList.length - 1; i >= 0; i--) {
          ...
          uploadFunBySys(
            file.raw,
            `voucher/db/${this.activityId}_${~~(Math.random() * 1000)}`,
            (url) => {
              console.log(url)
