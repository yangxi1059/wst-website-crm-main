...
<template>
  <div class="mentor-pay">
    <el-dialog :close-on-click-modal="false"
      :title="'支付'"
      :visible.sync="recommendCashierVisible"
      width="1200px"
...
export default {
  ...
  watch: {
    recommendCashierVisible: function (newData, oldData) {
      if (newData) {
  ...
  methods: {
    ...
    submit () {
      this.$refs.submitData.validate(valid => {
        ...
        const data = {}
        // if (!this.submitData.payId) {
        this.submitData.payRate = this.refundData.content.info.exchangeRate
        data.addList = [this.submitData]
        // } else {
        // data.uptList = [this.submitData];
        // }
        ...
        if (!this.file) {
          ...
          uploadFunBySys(this.file, 'voucher/recommend_cashier', url => {
            ...
            data.addList[0].voucherArr = [{ voucherName: this.file.name, voucherPath: url }]
            // }
