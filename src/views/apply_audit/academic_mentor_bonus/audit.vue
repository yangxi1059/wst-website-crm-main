<!--
 * @Author: kaan
 * @Date: 2021-04-08 11:05:16
 * @LastEditors: 杨曦
 * @LastEditTime: 2022-11-17 15:09:36
 * @Version:
 * @Description: 升学导师佣金
-->
...
<template>
  <div class="mentor-pay">
    ...
    <el-dialog :close-on-click-modal="false"
      :title="'升学导师佣金审核'"
      :visible.sync="academicMentorBonusVisible"
      width="1000px"
...
export default {
  ...
  watch: {
    academicMentorBonusVisible: function (newData, oldData) {
      if (newData) {
  ...
  methods: {
    Topage () {
      api.getApplyDetailByApplyId(this.applyData.applyId).then(res => {
        console.log('详情数据yx123123132', JSON.parse(res.data.apply.content))
        this.refundData = {
          pay: res.data.pay,
        ...
        // });
        /* 升学导师佣金默认为usd。此处注释不用
        if (this.refundData.content.info.exchangeRate !== "exchangeRateReplace") {
          this.rate = this.refundData.content.info.exchangeRate;
        }
        if (this.refundData.content.info.fundType == "usd") {
          this.refundData.mentorPaymentExtra.fundWageCny =
            Math.round(
              this.refundData.content.info.fundWage * this.rate * 100
            ) / 100;
        } else {
          this.refundData.mentorPaymentExtra.fundWageCny = this.refundData.content.info.fundWage;
        } */
        if (this.refundData.content.info.fundType == 'cny') {
          this.refundData.mentorPaymentExtra.fundWageCny = Math.round(this.refundData.content.info.fundWage * 100) / 100
        } else {
          this.refundData.mentorPaymentExtra.fundWageCny = Math.round(this.refundData.content.info.fundWage * this.rate * 100) / 100
        }
        let point = true
        res.data.approval.forEach(v => {
    ...
    submit () {
      this.$confirm('是否确认通过此审核?', '提示', {
        ...
        const data = {
          ...
          cny: this.refundData.mentorPaymentExtra.fundWageCny
        }
        console.log(data)
        api
          .setAuditRefund(data)
