<template>
  <div class="mentor-pay">
    <el-dialog :close-on-click-modal="false"
      :title="'全职导师上课申请'"
      :visible.sync="strategistListAuditVisible"
      width="1000px"
    ...
    >
      ...
      <el-row class="mb10" v-if="refundData.content">
        <span v-for="(item,i) in refundData.content.file" :key="i">
          <el-col :span="4" class="_item-name">课件 {{++i}}</el-col>
...
export default {
  ...
  watch: {
    strategistListAuditVisible: function (newData, oldData) {
      if (newData) {
