<!--
 * @Author: 库建华
 * @Date: 2019-08-14 16:15:32
 * @LastEditors: 杨曦
 * @LastEditTime: 2021-12-27 14:31:26
 * @Version:
 * @Description:
 -->
<template>
  <div class="mentor-pay">
    <el-dialog :close-on-click-modal="false"
      :title="'返佣支付'"
      :visible.sync="brokerageCashierVisible"
      width="1200px"
    ...
    >
      <el-card class="mb20">
        ...
        <el-row class="mb10" v-if="refundData.content">
          <span v-for="(item,i) in refundData.content.text" :key="i">
            <el-col :span="4" class="_item-name">{{item.label}}</el-col>
            <el-col :span="4" class="_item-value" :title="item.value">{{item.value}}</el-col>
          </span>
        </el-row>
        <el-row class="mb10">
...
export default {
  ...
  watch: {
    brokerageCashierVisible: function (newData, oldData) {
      if (newData) {
    ...
    submit () {
      this.$refs.submitData.validate(valid => {
        ...
        if (!this.file) {
          ...
          uploadFunBySys(this.file, 'voucher/brokerage_cashier', url => {
            console.log(url)
