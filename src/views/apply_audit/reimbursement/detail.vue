<template>
  <div class="mentor-pay">
    <el-dialog :close-on-click-modal="false"
      :title="'报销申请'"
      :visible.sync="reimbursementDetailVisible"
      width="1000px"
    ...
    >
      ...
      <el-row class="mb10" v-if="refundData.content">
        <el-col :span="4" class="_item-name">申请凭证</el-col>
        <el-col :span="10" class="_item-value">
      ...
      <el-row class="mb10">
        <el-col :span="4" class="_item-name">报销事由</el-col>
        <el-col :span="20" class="_item-value" :title="account" v-text="account">{{}}</el-col>
      </el-row>-->
      <el-row class="mb10">
...
export default {
  ...
  watch: {
    reimbursementDetailVisible: function (newData, oldData) {
      if (newData) {
        ...
        api.getApplyDetailByApplyId(this.applyData.applyId).then(res => {
          ...
          console.log('详情数据', res, this.refundData.content.file)
          this.account = ''
          this.account = this.refundData.content.info.reimbursementReason
          console.log(this.account, this.refundData.content.info.payAccount)
          this.totalRefund = ''
          this.totalRefund = this.refundData.content.info.reimbursementTotalWage
          this.approval = ''
