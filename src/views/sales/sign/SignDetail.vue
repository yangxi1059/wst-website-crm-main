<!--
 * @Author: kaan
 * @Date: 2021-06-08 14:20:42
 * @LastEditors: 杨曦
 * @LastEditTime: 2022-11-24 15:13:11
 * @Version:
 * @Description: rebate签约不再返佣
-->
<template>
    ...
    >
      ...
      <div class="create-cant-program-model" v-show="offerPList.programId">
        ...
        >
          ...
          <!-- <el-form-item label="官方价格($)" >
            <el-input-number :style="{width:widthInput}" v-model="baseProgram.basePriceUsd" :controls="false" size="mini" :min="0"></el-input-number>
          </el-form-item> -->
          <el-form-item  label="官方价格($)" >
            <el-input-number :disabled="!priceModify" :style="{width:widthInput}" v-model="baseProgram.basePriceUsd" :controls="false" size="mini" :min="0"></el-input-number>
          </el-form-item>
          ...
          <el-form-item label="等值人民币(￥)" prop>
            <!-- 原span标签内 v-if="baseProgram.currencyType === 'usd'" -->
            <span
              :style="{width:widthSpan}"
              class="programName"
          ...
          <el-form-item label="行业导师一对一（基础）" prop="basicMentorHour">
            <span
              :style="{width:widthSpan}"
              class="programName"
              v-if="baseProgram.basicMentorHour == -1"
            >不限</span>
            <el-input-number
              v-else
              @change="changeMentorHour"
              :disabled="!lessonHourModify"
              :style="{width:widthInput}"
              v-model="baseProgram.basicMentorHour"
              :controls="false"
              size="mini"
              :min="0"
            ></el-input-number>
          ...
          <el-form-item label="行业导师一对一（赠送）" prop="freeMentorHour">
            <span
              :style="{width:widthSpan}"
              class="programName"
              v-if="baseProgram.freeMentorHour == -1"
            >不限</span>
            <el-input-number
              v-else
              @change="changeMentorHour"
              :disabled="!lessonHourModify"
              :style="{width:widthInput}"
              v-model="baseProgram.freeMentorHour"
              :controls="false"
              size="mini"
              :min="0"
            ></el-input-number>
          ...
          <el-form-item  label-width="180px" label="1对多Office Hour" prop="officeHourStrategistSession">
            <span
              :style="{width:'140px'}"
              class="programName"
          ...
          <el-form-item label="系列课课时" prop="seriesHour">
            ...
            <el-input-number
              ...
              :disabled="!lessonHourModify"
              :style="{width:widthInput}"
              v-model="baseProgram.seriesHour"
              :controls="false"
              size="mini"
              :min="0"
            ></el-input-number>
          ...
          <el-form-item label="Live Lessons（新）" prop="liveLesson">
            ...
            <el-input-number
              ...
              :disabled="!lessonHourModify"
              :style="{width:widthInput}"
              v-model="baseProgram.liveLesson"
              :controls="false"
              size="mini"
              :min="0"
            ></el-input-number>
          ...
          <el-form-item label="全职导师文书修改次数" prop="applicationLetterModify">
            <span :style="{width:widthSpan}" class="programName">
              {{(baseProgram.applicationLetterModify == -1 ? '不限' : baseProgram.applicationLetterModify) || '无'}}
            </span>
          </el-form-item>
          <el-form-item label="行业导师文书修改次数" prop="mentorLetterModify">
            <span :style="{width:widthSpan}" class="programName">
              {{(baseProgram.mentorLetterModify == -1 ? '不限' : baseProgram.mentorLetterModify) || '无'}}
            </span>
          </el-form-item>
          ...
          <el-form-item label="优先内推" prop="priority">
            <span :style="{width:widthSpan}" class="programName">{{baseProgram.priority || '无'}}</span>
          </el-form-item>
          <el-form-item label="系列课（口语）" prop="hasEnglishSeries">
            <span :style="{width:widthSpan}" class="programName">{{baseProgram.hasEnglishSeries || '无'}}</span>
          </el-form-item>
          <el-form-item label="口语课程" prop="hasOral">
            <span :style="{width:widthSpan}" class="programName">{{baseProgram.hasOral || '无'}}</span>
          </el-form-item>
          ...
          <div class="program-model-title">申研项目</div>
            ...
            >
              ...
              <el-form-item
                label="Strategist:"
                prop="strategist"
                v-if="graduateProgram.signVip == 'Strategist' || graduateProgram.signVip == 'ALL'"
              >
                <el-select
                  :style="{width:widthInput}"
                  v-model="graduateProgram.strategist"
                  filterable
                  placeholder="请选择"
                >
                  <el-option
              ...
              <el-form-item
                label="Program Manager:"
                prop="services"
                v-if="graduateProgram.signVip == 'Program_Manager' || graduateProgram.signVip == 'ALL'"
              >
                <el-select
                  :style="{width:widthInput}"
                  v-model="graduateProgram.services"
                  filterable
                  placeholder="请选择"
                >
                  <el-option
...
export default {
  ...
  watch: {
    ...
    graduatePrice (v) {
      ...
      // pItem.totalPriceCny = parseFloat((v.originalGraduatePriceUsd * v.midRate ).toFixed(1));
      // pItem.totalPriceUsd = v.originalGraduatePriceUsd
    }
    // discountPrice(v){
    //   console.log("r66666")
    //   let pItem = this.collect
    //   pItem.originalPriceCny = parseFloat((v.originalPriceUsd * v.midRate ).toFixed(1));
    //   pItem.originalPriceUsd = v.originalPriceUsd
    //   pItem.totalPriceCny = parseFloat((v.originalPriceUsd * v.midRate ).toFixed(1));
    //   pItem.totalPriceUsd = v.originalPriceUsd
    //   this.entrust.entrustPrice = `${pItem.totalPriceCny}(￥) / ${pItem.totalPriceUsd}($)`
    // },
  },
  mounted () {
  ...
  methods: {
    ...
    async keepOrder (ko) {
      ...
      await apiDic.getRedisByKey(this.menteeId).then((res) => {
        ...
        this.transition = res.data.transition || this.transition
    ...
    async init () {
      ...
      if (this.offerPList.programId) {
        ...
        apiDic.getProgramDetailById(this.offerPList.programId).then((res) => {
          console.log(res.data, '获取求职项目合同模板')
    ...
    changeGraduateOption (v) {
      ...
      // this.changePrice(this.graduatePList)
    },
    ...
    async onlineNext () {
      ...
      const nvba = ['menteeDetail', 'transition', 'baseProgram', 'collectOffer', 'graduateProgram', 'collectGraduate', 'gift', 'invoice', 'collect']
    ...
    dealData () {
      ...
      obj = JSON.parse(JSON.stringify(this.transition))
      Object.getOwnPropertyNames(obj).forEach(function (key) {
        _this.programData[`transition${key.charAt(0).toUpperCase()}${key.slice(1)}`] = obj[key]
      })
      // 求职部分
    ...
    submitGG2 () {
      ...
      if (this.graduatePList.programId) {
        const graduateData = {
          ...
          strategist: this.graduateProgram.strategist,
          services: this.graduateProgram.services,
