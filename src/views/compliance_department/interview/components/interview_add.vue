 ...
 * @LastEditors: 杨曦
 * @LastEditTime: 2022-02-07 09:55:36
 * @Version:
 * @Description:
 -->
<template>
  <div class="hr-user-edit">
    <el-dialog :close-on-click-modal="false" :title="'新增'" :visible.sync="addVisible" width="900px" :before-close="close">
      ...
      >
        ...
        <el-form-item label="到面状态" prop="interviewStatus">
          <el-select
            :style="{width:widths}"
            filterable
        ...
        <el-form-item label="推荐人" v-if="userData.intervieweeResource == 'user_recommend' " prop="wstUserId">
          <el-select
            :style="{width:widths}"
            filterable
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        ...
        intervieweeResource: [{ required: true, message: '必填', trigger: 'blur' }]
      },
      users: [],
  ...
  watch: {
    addVisible: function (val) {
      if (val) {
        this.userData.manageBy = this.userInfo.userId
      }
    }
  },
  mounted () {
