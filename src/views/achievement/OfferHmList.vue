<template>
  <d2-container v-loading="loading">
		<div class="feedbackyx d2_container" ref="d2">
			<div class="search_page" ref="search">
				<div class="search">
					...
					<el-select
						...
						class="mr10"
						size="mini"
						v-model="fullTimeType"
						clearable
						placeholder="全职类型"
						@change="Topage()"
					>
						<el-option
							v-for="(item,i) in full_time_type"
							:key="i"
							:label="item.itemName"
							:value="item.itemValue"
						></el-option>
					...
					>
						<el-option
							v-for="(item,i) in internship_type"
							:key="i"
							:label="item.itemName"
							:value="item.itemValue"
						></el-option>
			...
			>
				...
				<el-table-column align="center" label="按钮功能区">
					<el-table-column align="center" width="110px" label="操作" v-if="roleInfo.includes(`offer_hm_list_setting`) || roleInfo.includes(`offer_hm_list_audio`)">
						<template slot-scope="scope">
							<div style="display:flex;justify-content:center">
								<!-- <el-button type="text" size="mini" v-if="roleInfo.includes(`offer_hm_list_setting`)" @click="setting(scope.row)">核验</el-button> -->
				...
				<el-table-column align="center" label="学员信息">
					<el-table-column  align="center" prop="menteeResume" label="简历">
						<template slot-scope="scope">
							<el-button
								size="mini"
								type="text"
								v-if="scope.row.menteeResume"
								@click="downloadResume(scope.row.menteeResume)"
							>查看</el-button>
						</template>
					</el-table-column>
					<el-table-column
				...
				<el-table-column align="center" label="offer信息">
					...
					<el-table-column
						...
						show-overflow-tooltip
					></el-table-column>
					<el-table-column align="center" prop="basicDivisionName" label="一级部门信息" width="120px"></el-table-column>
					<el-table-column
						min-width="100"
						align="center"
						prop="divisionName"
					...
					<el-table-column min-width="100" align="center" sortable prop="offerReceiveDate" label="offer获得日期" width="120px"></el-table-column>
				...
				<el-table-column align="center" label="工作信息">
					...
					<el-table-column align="center" prop="publicStatusName" label="可宣传">
						<template slot-scope="scope">
							<div style="color:red;font-weight: bold;" v-if="scope.row.publicStatus == '0'">{{scope.row.publicStatusName}}</div>
							<div v-else>{{scope.row.publicStatusName}}</div>
						</template>
					</el-table-column>
					...
					<el-table-column align="center" sortable prop="createTime" label="创建时间" width="120px" show-overflow-tooltip></el-table-column>
...
import setting from './components/setOfferHm.vue'
import checkDownLoad from './components/check_download.vue'
import applyAudio from './components/audioApply.vue'
...
export default {
  ...
  watch: {
    total: function (val, old) {
      	this.$nextTick(function () {
			...
			console.log(this.tableHeight,1111111111111)
		})
    },
  },
  data () {
    return {
	  offerListHeight: 'auto',
  ...
  methods: {
	...
	onResize(){
		if(this.$refs.d2.offsetHeight - this.$refs.search.offsetHeight < 400){
			this.tableHeight = 400 + 'px'
		}else{
			this.tableHeight = this.$refs.d2.offsetHeight - this.$refs.search.offsetHeight + 'px'
		}
		console.log(this.tableHeight,222222222222)
	},
	tableRowClassName({row, rowIndex}){
		...
		return ''
		console.log(row,rowIndex,444444444444444444)
	},
    init () {
      // 获取用户列表users
    ...
    Topage () {
      ...
      const data = {
		...
		fullTimeType: this.fullTimeType,
...
<style  >
.feedbackyx .redBg-row {
	background-color: rgba(252, 157, 154, 0.3);
  }
</style>
