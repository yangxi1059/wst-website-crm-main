 ...
 * @LastEditors: 杨曦
 * @LastEditTime: 2022-01-25 15:53:51
 * @Version: 1
 * @Description:
 -->
<template>
			...
			>
				...
				<el-table-column align="center" label="按钮功能区">
					<el-table-column
						...
						label="操作"
						width="110"
						v-if="roleInfo.includes(`offer_edu_hm_list_setting`) || roleInfo.includes(`offer_edu_hm_list_audio`)"
					>
						<template slot-scope="scope">
							<div style="display:flex;justify-content:center">
								<!-- <el-button type="text" size="mini" v-if="roleInfo.includes(`offer_edu_hm_list_setting`)" @click="setting(scope.row)">核验</el-button> -->
					...
					<el-table-column min-width="180" align="center" prop="offerVoucherMosaic" label="凭证">
						<template slot-scope="scope">
							...
							<el-button
								size="mini"
								type="text"
								v-if="roleInfo.includes(`offer_edu_hm_download_list`) && scope.row.offerVoucherMosaic"
					...
					<el-table-column min-width="180" align="center" prop="praiseVoucherMosaic" label="好评图">
						<template slot-scope="scope">
							...
							<el-button
								size="mini"
								type="text"
								v-if="roleInfo.includes(`offer_edu_hm_download_list`) && scope.row.praiseVoucherMosaic"
...
import setting from './components/setEduOfferHm.vue'
...
export default {
  ...
  methods: {
    ...
    // 查看
    download (path) {
      if (!this.roleInfo.includes('offer_edu_hm_list_view_hm')) {
        this.$message('无权限')
        return
      }
      console.log(path)
    ...
    download2 (path) {
      if (!this.roleInfo.includes('offer_edu_hm_list_down_hm')) {
        this.$message('无权限')
        return
      }
      console.log(path)
    ...
    // 下载
    downloadD (data) {
      if (!this.roleInfo.includes('offer_edu_hm_list_img_view_hm')) {
        this.$message('无权限')
        return
      }
      const params = {
    ...
    downloadD2 (data) {
      if (!this.roleInfo.includes('offer_edu_hm_list_img_down_hm')) {
        this.$message('无权限')
        return
      }
      const params = {
