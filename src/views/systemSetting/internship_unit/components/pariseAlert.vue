 ...
 * @LastEditors: 杨曦
 * @LastEditTime: 2022-02-16 10:58:45
 * @Version:
 * @Description:
-->
<template>
  <div class="praiseAlert">
    <el-dialog :visible.sync="praiseVisible" width="1000px" :close-on-click-modal="false"
    :title="`实习【${this.internshipId2.internshipName || '无'}】的好评图`"
    :before-close="close">
      <el-card >
...
export default {
  ...
  watch: {
    praiseVisible: function (val, old) {
      if (val) {
        this.Topage()
      }
    }
  ...
  methods: {
    Topage () {
      api.getInternshipPraise(this.internshipId2.internshipId).then(res => {
    ...
    uploadFileAxios (file, fileList) {
      ...
      uploadFun(
        file.file,
        `internShipPraise/${this.internshipId2.internshipId}`,
        p => {
          that.progress = p
        },
        url => {
          ...
          console.log(data)
          api.addInternshipPraise(data).then(res => {
    ...
    deleteFile (Id) {
      console.log(Id)
      api.deleteInternshipPraise(Id).then(res => {
        console.log(res)
        this.Topage()
        this.$emit('submit')
      })
