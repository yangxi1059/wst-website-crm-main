<template>
  <div class="brokerage lesson_pay_apply_container">
    <el-dialog :close-on-click-modal="false" title="内推面试费用申请" :visible.sync="jobInterviewFeeVisible" width="800px" :before-close="close">
...
export default {
  ...
  watch: {
    jobInterviewFeeVisible: function (val) {
      if (val) {
        ...
        axios.getAuditorListByApplyType('job_interview_fee').then(res => {
  ...
  methods: {
    ...
    providerDetail () {
      api.getInternalJobDetail(this.providerId).then(res => {
        ...
        this.fee = res.data.interviewFee
        this.feeType = res.data.interviewFeeType
    ...
    submit () {
      ...
      console.log(account)
      const data = {
        keyId: this.interviewDetail.pkId,
        applyType: 'job_interview_fee',
        applyTitle: `${this.menteeDetail.menteeName}的内推面试费用申请`,
        copyTo: copyTo,
        approval: approval,
        content: {
          file: [],
          ...
          info: {
            ...
            resultId: this.interviewDetail.pkId,
