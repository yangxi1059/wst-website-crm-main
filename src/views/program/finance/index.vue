 ...
 * @LastEditors: 杨曦
 * @LastEditTime: 2022-02-16 11:14:46
 * @Version: 1
 * @Description:
 -->
<template>
  <d2-container v-loading="loading">
    <div class="dictionary">
        ...
        <div class="search">
            <el-input
            class="mr10"
            v-if="roleInfo.includes(`finance_search`)"
            size="mini"
            style="width:150px"
            v-model="search"
            clearable
            ...
            icon="el-icon-plus"
            class=""
            v-if="roleInfo.includes(`finance_new`)"
            size="mini"
            plain
            @click="newdic"
            >新增</el-button>
            <el-button
            v-if="roleInfo.includes(`finance_sort`)"
            class=""
            size="mini"
            type="primary"
            @click="sortTableBtn"
        ...
        <pagination
            v-if="roleInfo.includes(`finance_page`)"
            :total="total"
        ...
        <el-table :data="tableData" size="mini" highlight-current-row >
            <el-table-column align="center" label="详情" width="200px">
                ...
                <el-button
                    type="text"
                    size="mini"
                    v-if="roleInfo.includes(`finance_edit`)"
                ...
                <el-button
                    type="text"
                    size="mini"
                    v-if="roleInfo.includes(`finance_pic`)"
                    @click="toImg(scope.row.programId)"
                    class="el-icon-picture-outline"
                >图片</el-button>
                <el-button
                    type="text"
                    v-if="roleInfo.includes(`finance_del`)"
                    size="mini"
            ...
            <el-table-column align="center" prop="financeHour" label="财商导师一对一课时数">
                <template slot-scope="scope">{{scope.row.financeHour == -1 ? '不限' :scope.row.financeHour}}</template>
            </el-table-column>
            ...
            <el-table-column align="center" prop="brokerageRate"  label="项目固定提成比例(%)">
                <template slot-scope="scope">
                <span v-if="scope.row.brokerageRate == '0'">非固定</span>
                <span v-else>固定{{scope.row.brokerageRate}}%</span>
                </template>
            </el-table-column>
        </el-table>
        <editProgram
...
export default {
  ...
  methods: {
    initPage () {
      api.getProgramDic('finance').then((res) => {
    ...
    newdic () {
      this.ruleForm.programType = 'finance'
      this.programId = ''
