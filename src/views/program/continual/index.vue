    ...
    <div class="dictionary">
        <div class="search_page">
            <div class="search">
                <el-input
                class="mr10"
                v-if="roleInfo.includes(`continual_search`)"
                size="mini"
                style="width:150px"
                v-model="search"
                clearable
                ...
                icon="el-icon-plus"
                class=""
                v-if="roleInfo.includes(`continual_new`)"
                size="mini"
                plain
                @click="newdic"
                >新增</el-button>
                <el-button
                v-if="roleInfo.includes(`continual_sort`)"
                class=""
                size="mini"
                type="primary"
                @click="sortTableBtn"
            ...
            <pagination
                v-if="roleInfo.includes(`continual_page`)"
                :total="total"
        ...
        <el-table-column align="center" label="详情" width="200px">
            ...
            <el-button
                type="text"
                size="mini"
                v-if="roleInfo.includes(`continual_edit`)"
            ...
            <el-button
                type="text"
                size="mini"
                v-if="roleInfo.includes(`continual_pic`)"
                @click="toImg(scope.row.programId)"
                class="el-icon-picture-outline"
            >图片</el-button>
            <el-button
                type="text"
                v-if="roleInfo.includes(`continual_del`)"
                size="mini"
        ...
        <el-table-column align="center" prop="mentorHour" label="行业导师一对一"></el-table-column>
...
import detailProgram from './components/detail_program.vue'
import editProgram from './components/edit_program.vue'
import sortTable from './components/sortTable.vue'
import imgUpload from './components/img_upload.vue'
import { mapState } from 'vuex'

export default {
  components: { inputNumber, detailProgram, editProgram, sortTable, imgUpload },
  mixins: [mixins],
  computed: {
    ...mapState('role', [
      'roleInfo'
  ...
  methods: {
    ...
    viewContract (contractContent, contractTitle) {
      console.log(contractContent, contractTitle, 'yx111111111111111111')
      const submitData = {
    ...
    addContract () {
      ...
      console.log(this.contractData, 'yx11111111111111111111')
