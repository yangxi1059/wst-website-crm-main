 ...
 * @LastEditors: 杨曦
 * @LastEditTime: 2022-02-16 11:22:53
 * @Version: 1
 * @Description:
 -->
<template>
  <d2-container v-loading="loading">
    <div class="dictionary">
        ...
        <div class="search">
            <el-input
            class="mr10"
            v-if="roleInfo.includes(`jc_pro_search`)"
            size="mini"
            style="width:150px"
            v-model="search"
            clearable
            ...
            icon="el-icon-plus"
            class=""
            v-if="roleInfo.includes(`jc_pro_new`)"
            size="mini"
            plain
            @click="newdic"
            >新增</el-button>
            <el-button
            v-if="roleInfo.includes(`jc_pro_sort`)"
            class=""
            size="mini"
            type="primary"
            @click="sortTableBtn"
        ...
        <pagination
            v-if="roleInfo.includes(`jc_pro_page`)"
            :total="total"
        ...
        <el-table :data="tableData" size="mini" highlight-current-row >
            <el-table-column align="center" label="详情" width="200px">
                ...
                <el-button
                    type="text"
                    size="mini"
                    v-if="roleInfo.includes(`jc_pro_edit`)"
                ...
                <el-button
                    type="text"
                    size="mini"
                    v-if="roleInfo.includes(`jc_pro_pic`)"
                    @click="toImg(scope.row.programId)"
                    class="el-icon-picture-outline"
                >图片</el-button>
                <el-button
                    type="text"
                    v-if="roleInfo.includes(`jc_pro_del`)"
                    size="mini"
...
import detailProgram from '../continual/components/detail_program_pro.vue'
...
export default {
  ...
  methods: {
    initPage () {
      api.getProgramDic('jcxy').then((res) => {
    ...
    newdic () {
      this.ruleForm.programType = 'jcxy'
      this.programId = ''
