 ...
 * @LastEditors: 杨曦
 * @LastEditTime: 2022-11-14 16:24:23
 * @Version:
 * @Description:
-->
<template>
	<div>
		...
		>
			<el-tabs v-model="activeName" type="card" @tab-click="handleClick">
				<el-tab-pane label="基本信息" name="first">
					<div>
						<el-row>
							...
							<el-col :span="8" class="_item-value">{{addData.codes || '暂无'}}</el-col>
							...
							<el-col :span="8" class="_item-value">{{addData.pretalkTypeName || '暂无'}}</el-col>
							<el-col :span="4" class="_item-name">姓名:</el-col>
							<el-col :span="8" class="_item-value">{{addData.pretalkName || '暂无'}}</el-col>
              ...
              <el-col :span="4" class="_item-name">可带行业:</el-col>
							<el-col :span="8" class="_item-value">{{addData.trackName || '暂无'}}</el-col>
							<el-col :span="4" class="_item-name">可带地区:</el-col>
							<el-col :span="8" class="_item-value">{{addData.countryName || '暂无'}}</el-col>
							<el-col :span="4" class="_item-name">是否启用:</el-col>
							<el-col :span="8" class="_item-value">{{addData.pretalkStatusName || '暂无'}}</el-col>
				...
				<el-tab-pane label="账户" name="second">
					<el-form :inline="true" size="mini" v-if="addData.pretalkType == 'other'" >
						<el-form-item label="">
					...
					<el-card class="box-card box-cardTe mb20" :class="item.payStatus == '1'?'disabled1':''"  size="mini"  v-for="(item,i) in menteeList" :key="item.accountId">
						<div class="mb20" style="display:flex;justify-content:space-between">
							...
							<div style="">
								<el-button size="mini" type="success" v-if="addData.pretalkType == 'other' && (item.payStatus == '0' && item.priority == '0' )  " @click="defaultItem(item)">设为默认</el-button>
								<el-button size="mini" type="primary" v-if="addData.pretalkType == 'other'" @click="editItme(i)">编辑</el-button>
								<el-button size="mini" type="warning" v-if="addData.pretalkType == 'other' && item.payStatus == '0'" @click="warningItem(item)">禁用</el-button>
								<el-button size="mini" type="success" v-if="addData.pretalkType == 'other' && item.payStatus == '1'" @click="successItem(item)">启用</el-button>
								<el-button size="mini" type="danger" v-if="addData.pretalkType == 'other'" @click="deleteItme(item)">删除</el-button>
							</div>
						</div>
        ...
        <el-tab-pane label="绑定学员" name="tired">
					<div>
            ...
            <el-card box-card box-cardTe size="mini" class="mb20" v-for="(item,i) in menteeArr" :key="item.menteeId+i">
              <el-col :span="4" class="_item-name">学员ID:</el-col>
							...
							<el-col :span="4" class="_item-name">微信名:</el-col>
							<el-col :span="8" class="_item-value">{{item.wxName || '暂无'}}</el-col>
              <el-col :span="4" class="_item-name">记录人:</el-col>
							<el-col :span="8" class="_item-value">{{item.createByName || '暂无'}}</el-col>
              <el-col :span="4" class="_item-name">记录时间:</el-col>
							<el-col :span="8" class="_item-value">{{item.createTime || '暂无'}}</el-col>
            </el-card>
					</div>
				</el-tab-pane>
			</el-tabs>
			<div slot="footer" class="dialog-footer">
        <el-button v-if="activeName == 'first'" @click="deleteKolDetail" type="danger">删 除</el-button>
        <el-button v-if="activeName == 'first' && roleInfo.includes(`pretalk_edit`) " @click="editPretalk" type="primary" >编 辑</el-button>
    ...
    <el-dialog :close-on-click-modal="false"
		...
		<editPretalk :editVisible="editVisible" :data="data" @close="closeKol" @submit="submitKol" />
	</div>
</template>
...
import editPretalk from './editPretalk.vue'
export default {
  components: { editPretalk },
  mixins: [mixins],
  name: 'addKol',
  props: {
    detailVisible: {
  ...
  methods: {
    ...
    toPage () {
      api.detailPretalk(this.pretalkId).then(res => {
        console.log(res.data)
        this.addData = res.data
      })
      this.TomenteeArr()
      // api.applyArr(this.pretalkId).then(res => {
      //   console.log(res.data,222)
      //   this.applyArr = res.data;
      // })
    },
    TomenteeArr () {
      api.menteeArrPretalk(this.pretalkId).then(res => {
    ...
    deleteKolDetail () {
      this.$confirm('此操作将永久删除该Pretalk, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        api.deletePretalk(this.addData.pretalkId).then(() => {
          this.$message({
            type: 'success',
            message: '删除成功!'
          })
    ...
    Topage () {
      if (this.addData.pretalkType == 'other') {
        api.getCooperatorPaymentListByCooperatorIdNew(this.addData.pretalkId).then(res => {
          console.log(res.data)
      ...
      } else if (this.addData.pretalkType == 'mentor') {
      ...
      } else if (this.addData.pretalkType == 'mentee') {
        api3.getCooperatorPaymentListByCooperatorIdNew(this.addData.menteeId).then(res => {
          this.menteeList = JSON.parse(JSON.stringify(res.data))
          console.log('学员支付账户列表', res)
          this.settings.data = []
          this.settings.data = res.data
    ...
    deleteItme (row) {
      ...
      this.$confirm('此操作将永久删除此账户, 是否继续?', '提示', {
        ...
        api.deleteCommonPaymentNew(row.accountId).then(res => {
    ...
    addPayWay2 () {
      this.$confirm('是否设置为默认账户 ？', '提示', {
        ...
        const data = this.accountData
        data.pretalkId = this.pretalkId
        console.log(data)
        api.addPretalkPay(data).then(res => {
          console.log(res)
          this.Topage()
          this.beforePayWayItemClose()
          // this.check(res)
    ...
    defaultItem (row) {
      ...
      this.$confirm('此操作将此账户设为默认支付方式, 是否继续?', '提示', {
        ...
        data.userId = this.pretalkId
        const dataNew = data
        api.uptCommonPaymentNew(dataNew).then(res => {
          console.log(res)
          this.Topage()
          // this.check(res)
          this.$message({
    ...
    addPretalk (menteeId) {
      ...
      if (menteeId) {
        api2.getMenteeDataByMenteeId(menteeId).then(() => {
          const data = {
            menteeId: menteeId,
            pretalkId: this.pretalkId
          }
          api.addMenteePretalk(data).then((res) => {
            if (res.code == '200') {
    ...
    newUser () {
      this.$loading()
      api.searchMenteeMatch(this.menteeId).then(res => {
        console.log(res.data)
