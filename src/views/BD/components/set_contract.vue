    ...
    >
      ...
      >
        ...
        <el-form-item label="草拟合同">
          ...
          >查看</el-button>
          <el-upload
            v-if="!cacheContractData.contractId"
            action
            class="upload-btn"
            ref="upload1"
            :auto-upload="false"
        ...
        <br />
        <el-form-item label="正式合同" v-if="cacheContractData.contractId">
          <el-button
            size="mini"
            type="text"
...
export default {
      ...
      if (val) {
        if (this.contractData.contractId) {
          ...
          this.cacheContractData = {
            cooperatorId: this.contractData.cooperatorId,
            cooperatorName: this.contractData.cooperatorName,
            contractStartDate: null,
            contractEndDate: null,
            contractDuration: null,
            contractSignDate: null,
  ...
  methods: {
    ...
    initPage () {
      api
        .getCooperatorContractDetailById(this.contractData.contractId)
        .then((res) => {
          console.log(res.data)
          this.cacheContractData = res.data
          this.cooperationArr = res.data.contentArr
          this.contractStatusName = res.data.contractStatusName
          this.attachmentArr = res.data.attachment
            ? res.data.attachment.split(',').map((v) => {
    ...
    applySubmit () {
      ...
      const data = {
        ...
        applyTitle: `BD合作商[${this.cacheContractData.cooperatorName}]的合同[${this.cacheContractData.contractName}]申请`,
        copyTo: copyTo,
        approval: approval,
        content: {
          file: [
