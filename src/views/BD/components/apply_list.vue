<template>
  <div class="mentor-pay">
    ...
    >
      <el-card class="mb20">
        <el-row class="mb10">
          <el-col :span="4" class="_item-name">草拟合同</el-col>
          <el-col :span="20" class="_item-value">
            <el-button @click="download(contractInfo.contractVoucher)" size="mini">查看</el-button>
          </el-col>
          <el-col :span="4" class="_item-name">正式合同</el-col>
          <el-col :span="20" class="_item-value">
            <el-button @click="download(contractInfo.formalContractVoucher)" size="mini">查看</el-button>
          </el-col>
          <div v-if="contractInfo.attachment">
            <span v-for="(item,i) in contractInfo.attachment.split(',')" :key="i">
              <el-col :span="4" class="_item-name">附件</el-col>
              <el-col :span="20" class="_item-value">
                <el-button @click="download(item)" size="mini">{{item.split('/').pop()}}</el-button>
              </el-col>
            </span>
          </div>
        </el-row>
  ...
  watch: {
    applyListVisible: function (newData) {
      if (newData) {
        this.$loading()
        api
          .getContractPayApplyListByContractId(this.contractData.contractId)
          .then((res) => {
            ...
            //   return;
            // }
            const resData = res.data.contractFundApplyArr
