<!--
 * @Author: 库建华
 * @Date: 2020-05-28 14:08:08
 * @LastEditors: 杨曦
 * @LastEditTime: 2022-02-21 13:37:20
 * @Version: 1
 * @Description:海外社团
-->
<template>
  <div class="hr-user-edit">
    ...
    >
      ...
      >
        ...
        <el-form-item label="是否目标学校" prop="targetSchoolStatus">
          <el-select
            :style="{width:widths}"
            filterable
            v-model="cacheCooperatorData.targetSchoolStatus"
            placeholder="请选择"
          >
            <el-option
        ...
        <el-form-item label="公众号ID" prop="officialAccountId">
          <el-input
            :style="{width:widths}"
        ...
        <el-form-item label="公众号名称" prop="officialAccountName">
          <el-input
            :style="{width:widths}"
        ...
        <el-form-item label="附件" prop="voucher">
          <el-button
            size="mini"
            type="text"
            v-if="cacheCooperatorData.voucher"
            @click="download(cacheCooperatorData.voucher)"
          >查看</el-button>
          <el-button
            size="mini"
            type="text"
            v-if="cacheCooperatorData.voucher"
            @click="downloadD(cacheCooperatorData.voucher)"
          >下载</el-button>
          <el-upload
            action
            class="upload-btn"
...
import { uploadFunBySys, downloadFun, downloadFunD } from '@/libs/file'
...
export default {
  ...
  data () {
    return {
      ...
      cacheCooperatorData: {
        ...
        other: null,
        schoolId: null,
        targetSchoolStatus: null,
        officialAccountId: null,
        officialAccountName: null,
        voucher: null
      },
      loading: false,
      brokerage_status: [],
  ...
  methods: {
    ...
    submit () {
      ...
      this.$refs.cacheCooperatorData.validate(valid => {
        ...
        if (this.fileList.length) {
          ...
          uploadFunBySys(file.raw, 'cooperator/voucher', url => {
            console.log(url)
            this.submitA(url)
          })
        } else {
    ...
    submitA (url) {
      this.cacheCooperatorData.voucher = url
      const data = {}
