 ...
 * @LastEditors: 杨曦
 * @LastEditTime: 2022-11-22 14:02:06
 * @Version:
 * @Description:
-->
  ...
  <div class="inputInformation">
    <div>
      ...
      >
        ...
        <div  style="text-align:right" v-if="taskDetail.taskStatus != 'finish' && taskDetail.taskStatus != 'cancel'">
          <el-button style="margin-right:40px" type="primary" @click="submit(false)">确 定</el-button>
        </div>
        </div>
      </el-form>
    </div>
...
export default {
  ...
  watch: {
    detail2Visible: function (newData) {
      if (newData) {
        this.mentorData1 = JSON.parse(JSON.stringify(this.mentorData0))
        console.log('mentorData1', this.mentorData1, this.trackCascaderDic, this.cityCascaderDic, this.companyCascaderDic)
        this.track = this.mentorData1.careerTrack
          ? this.mentorData1.careerTrackName.join()
          : ''
        this.mentorData1.company &&
          this.companyCascaderDic.forEach(e => {
            e.children.forEach(v => {
              if (v.value == this.mentorData1.company[0]) {
                this.mentorData1.company.unshift(e.value)
              }
            })
          })
        this.mentorData1.location &&
          this.cityCascaderDic.forEach(e => {
            e.children.forEach(v => {
              if (v.value == this.mentorData1.location[0]) {
                this.mentorData1.location.unshift(e.value)
              }
            })
          })
        this.mentorData1.division &&
          this.trackCascaderDic.forEach(e => {
            e.children.forEach(v => {
              if (v.value == this.mentorData1.division[0]) {
                this.mentorData1.division.unshift(e.value)
              }
            })
          })
        const tutoringArr = []
        this.mentorData1.tutoringSubject && this.mentorData1.tutoringSubject.forEach(i => {
        ...
        const arr2 = []
        this.mentorData1.careerTrack && this.mentorData1.careerTrack.forEach(item => {
        ...
        this.mentorData1.careerTrack = arr2
        this.mentorData1.tutoringSubject = tutoringArr
        this.mentorData = this.mentorData1
  ...
  methods: {
    initNew () {
      ...
      this.trackCascaderDic = util.sessions.get('trackCascaderDic')
      this.cityCascaderDic = util.sessions.get('cityCascaderDic')
      this.companyCascaderDic = util.sessions.get('companyCascaderDic')
    ...
    submit (typeStatus) {
      ...
      this.$refs.mentorData.validate(valid => {
        ...
        api.setMentorDataTask(data).then(({ data }) => {
          console.log('提交导师信息', data)
