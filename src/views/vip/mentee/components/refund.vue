 ...
 * @LastEditors: 杨曦
 * @LastEditTime: 2023-03-01 14:57:56
 * @Version:
 * @Description:
 -->
<template>
  <div class="refund">
    <el-dialog title="退款" :close-on-click-modal="false" :visible.sync="refundVisible" width="1200px" :before-close="close">
        ...
        <el-card class="mb20">
          ...
          <el-row>
            ...
            <el-col :span="20" class="mentee-detail-value">
              <el-input maxlength="200" show-word-limit type="textarea" :autosize="{ minRows: 2, maxRows: 4}" size="mini" v-model="account"></el-input>
            </el-col>
          </el-row>
          <el-row class="mt10">
            <el-col :span="3" class="mentee-detail-name">退款原因</el-col>
            <el-col :span="20" class="mentee-detail-value">
              <el-input maxlength="1000" show-word-limit type="textarea" :autosize="{ minRows: 2, maxRows: 4}" size="mini" v-model="note"></el-input>
            </el-col>
          </el-row>
          <el-row class="mb10" >
            <el-col :span="3" class="_item-name">Salse绩效</el-col>
            <el-col :span="6" class="_item-name">
              <el-switch
                :disabled="true"
                v-model="salesCommissionFlag"
                active-value="1"
                active-text="扣绩效"
                inactive-value="0"
                inactive-color="#45a"
          ...
          <el-row class="mb10">
            <el-col :span="3" class="_item-name">VIP绩效</el-col>
            <el-col :span="20" class="_item-name">
              <el-radio v-model="vipId" label="">不扣</el-radio>
              <el-radio v-model="vipId" v-for="(item,i) in vipArr" :key="i" :label="item.userId">{{item.userName}}</el-radio>
            </el-col>
          </el-row>
        </el-card>
...
export default {
  ...
  watch: {
    refundVisible: function (val) {
      if (val) {
        ...
        api.getProgramListByOrderIdNew(this.orderId).then(res =>{
          console.log(res)
          this.loading = false
          this.orderData = res.data.signArr
          this.billList = res.data.billArr
          this.vipArr = res.data.vipArr
        })
  ...
  methods: {
    ...
    submit () {
      ...
      let str = ''
      for (let i = 0; i < this.vipArr.length; i++) {
        if (this.vipId == this.vipArr[i].userId) {
          str = this.vipArr[i].userName
        }
      }
      const data = {
        ...
        content: {
          ...
          text: [
            ...
            {
              label: '扣除Sales绩效',
              value: this.salesCommissionFlag == '1'?'扣绩效':'不扣绩效'
            },
            {
              label: '扣除VIP绩效',
              value: str || '不扣'
          ...
          info: {
            ...
            note: this.note,
            salesCommissionFlag: this.salesCommissionFlag,
            vipId: this.vipId,
            commisionFlag: false,
            kpiFlag: false,
            program: [],
