...
export default {
  ...
  actions: {
    ...
    async openedChildrenUpdate({ state, commit, dispatch }, { fullPath,path }) {
      // 更新页面列表某一项
      let tagName = ""
      state.opened.forEach(async (page, index) => {
        if(!page.path){return}
        if (page.path.indexOf(path) > -1 && page.path !== path){
     ...
     */
    async open ({ state, commit, dispatch }, { name, params, query, fullPath, meta, path }) {
      // 已经打开的页面
      const opened = state.opened
      // 判断此页面是否已经打开 并且记录位置
      let pageOpendIndex = 0
      const pageOpend = opened.find((page, index) => {
        const same = (page.path === path || page.fullPath === fullPath)
        pageOpendIndex = same ? index : pageOpendIndex
        return same
      })
      
      // 判断关闭子页面
      if(pageOpend && JSON.stringify(pageOpend.query) !== JSON.stringify(query) ){
        // 打开已有页面 但参数不同，关闭所有子页面
        await dispatch('openedChildrenUpdate', {
          fullPath,path
        })
      }else if(!pageOpend && query !=={} && meta.closeChild){
        // 打开未有页面 根据路由参数closeChild判断是否关闭子页面
        await dispatch('openedChildrenUpdate', {
          fullPath,path
        })
      }
  ...
  mutations: {
    setTimeLogOut (state) {
      state.timeLogOut = 5400
    },
    deleteTimeLogOut (state) {
      state.timeLogOut = state.timeLogOut - 1
    },
    /**
     * @class keepAlive
